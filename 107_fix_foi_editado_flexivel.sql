-- ================================================================
-- FIX: Permitir que pedidos com "foi_editado = TRUE" recebam novos Upsells/CCs
-- Regra atualizada a pedido: A marcação "Editado" deve proteger apenas dados do cliente (endereço, etc).
-- Edições no nome do produto/pacote NÃO devem marcar como editado.
-- ================================================================

-- 1. Atualizar função de edição de descrição para NÃO travar o pedido
...
BEGIN
  ...
  SET 
    ...
    -- foi_editado = true, -- REMOVIDO: Mudar a descrição não deve bloquear novas consolidações
    updated_at = NOW()
  WHERE id = p_id;
...
-- 2. Atualizar Consolidação para permitir Pais com "foi_editado=TRUE"
...
BEGIN
    ...
    -- SOFT LOCKED: Pedidos que têm foi_editado = TRUE. 
    -- Agora eles NÃO são excluídos do loop principal de busca de filhos!
    -- Eles são apenas excluídos da DELEÇÃO automática (pois queremos preservar as edições de endereço).
    CREATE TEMP TABLE _locked_ids AS
    SELECT id FROM pedidos_consolidados_v3
    WHERE foi_editado = TRUE;

    -- Deletar apenas o que NÃO está etiquetado E NÃO está editado.
    DELETE FROM pedidos_consolidados_v3 c
    WHERE id NOT IN (SELECT id FROM _hard_locked_ids) 
      AND id NOT IN (SELECT id FROM _locked_ids);

    CREATE TEMP TABLE _processed (pedido_id UUID PRIMARY KEY);
    
    -- Ignorar códigos que já pertencem a pedidos etiquetados (Hard Locked)
    ...
    LOOP
        ...
        ON CONFLICT (codigo_transacao) DO UPDATE SET
            ...
            -- ARRAY UNION p/ preservar produtos unidos manualmente por "unificar_pedidos"
