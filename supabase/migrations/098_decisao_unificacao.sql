-- ================================================================
-- MIGRATION 098: SISTEMA DE DECISÃO DE DIVERGÊNCIAS
-- ================================================================

CREATE TABLE IF NOT EXISTS decisoes_unificacao (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    hash_filho TEXT UNIQUE NOT NULL,
    hash_pai TEXT NOT NULL,
    acao TEXT CHECK (acao IN ('UNIFICAR', 'SEPARAR')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Atualizar a função para considerar estas decisões
...
BEGIN
    ...
    LOOP
        ...
        v_nome_oferta := TRIM(REGEXP_REPLACE(pedido_pai.offer_name, '\s*-\s+\w+\s*$', '', 'i'));
        ...
        LOOP
            -- Verificar se já existe decisão manual
            ...
            IF v_decisao_existente = 'SEPARAR' THEN
                CONTINUE; -- Ignora este filho para este pai específico
            END IF;

            -- Se não for 'UNIFICAR' forçado, verifica divergência
            IF v_decisao_existente IS NULL AND 
               (LOWER(TRIM(rec.customer_name)) != LOWER(TRIM(pedido_pai.customer_name)) OR
                ...
                v_lista_divergencias := v_lista_divergencias || jsonb_build_object(
                    ...
                    'quantidade', rec.item_quantity
                );
                CONTINUE;
            END IF;

            -- Unifica (Automático ou Decisão 'UNIFICAR')
            ...
            IF UPPER(rec.offer_name) ~* 'ORDER\s*BUMP' THEN
...
-- RPC para salvar decisão
CREATE OR REPLACE FUNCTION salvar_decisao_unificacao(p_hash_filho TEXT, p_hash_pai TEXT, p_acao TEXT)
RETURNS void AS $$
BEGIN
    INSERT INTO decisoes_unificacao (hash_filho, hash_pai, acao)
    VALUES (p_hash_filho, p_hash_pai, p_acao)
    ON CONFLICT (hash_filho) DO UPDATE SET acao = EXCLUDED.acao, hash_pai = EXCLUDED.hash_pai;
    
    -- Forçar re-consolidação para aplicar a decisão
    PERFORM consolidar_pedidos_ticto();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
