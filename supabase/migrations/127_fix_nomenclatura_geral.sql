-- ================================================================
-- MIGRATION 127: Fix Nomenclatura Geral
-- ================================================================
-- PROBLEMAS RESOLVIDOS:
--   1. Produtos não-DP (Bela Lumi, Bela Forma, SEKA, etc) 
--      estavam aparecendo com "1 FRASCO", "POTES", "CAIXAS"
--      pois a normalização ignorava as quantidades deles.
--      Fix: Extração de quantidade ("(\d+)") replicada
--           para todos os demais produtos, formatando como "[Qtd] UN".
--
-- ================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- FIX 127: extração de quantidade e formatação generalizada
        -- ──────────────────────────────────────────────────────────
        DECLARE
        ...
        BEGIN
            IF pedido_pai.sigla = 'DP' THEN
                ...
                END IF;
            ELSE
                -- FIX 127: Para demais produtos (Bela Lumi, Bela Forma, etc) padronizar retirando FRASCO
                v_raw_offer := NULLIF(TRIM(pedido_pai.offer_name), '');
                IF v_raw_offer IS NOT NULL THEN
                    IF v_raw_offer ~* 'LEVE\s+(\d+)' THEN
                        v_qtd := SUBSTRING(v_raw_offer FROM '(?i)LEVE\s+(\d+)');
