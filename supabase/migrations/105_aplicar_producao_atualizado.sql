...
-- Funcionalidade: Unificação de Pedidos com Mesmo Endereço ou CPF
-- ================================================================
-- Instruções:
...
-- 1. Função para encontrar pedidos com inconsistência (dentro de 5 dias)
...
BEGIN
    -- Buscar dados do pedido de referência
    SELECT 
        COALESCE(p.cpf_cliente, p.cpf, ''),
        LOWER(TRIM(COALESCE(p.nome_cliente, ''))),
        chave_endereco(p.cep, p.cidade, p.estado, COALESCE(p.logradouro, p.rua), p.numero),
        p.data_venda,
        LOWER(COALESCE(p.email_cliente, p.email, ''))
    INTO v_cpf_referencia, v_nome_referencia, v_chave_referencia, v_data_referencia, v_email_referencia
    FROM pedidos p
    WHERE id = p_pedido_id;
    
    -- Retornar pedidos com:
    -- 1) MESMO ENDEREÇO + CPF/NOME DIFERENTE (fraude de endereço)
    -- 2) MESMO CPF + ENDEREÇO DIFERENTE (cliente com múltiplos endereços)
    RETURN QUERY
    SELECT 
        p.id AS pedido_id,
        p.codigo_transacao,
    ...
    WHERE p.id != p_pedido_id
      ...
      AND ABS(EXTRACT(EPOCH FROM (p.data_venda - v_data_referencia))) <= 432000
      AND (
          -- CASO 1: MESMO ENDEREÇO + CPF/NOME DIFERENTE
          (
              chave_endereco(p.cep, p.cidade, p.estado, COALESCE(p.logradouro, p.rua), p.numero) = v_chave_referencia
              AND (
                  normalizar_documento(COALESCE(p.cpf_cliente, p.cpf, '')) != normalizar_documento(v_cpf_referencia)
                  OR LOWER(TRIM(COALESCE(p.nome_cliente, ''))) != v_nome_referencia
              )
          )
          -- CASO 2: MESMO CPF + ENDEREÇO DIFERENTE
          OR (
              normalizar_documento(COALESCE(p.cpf_cliente, p.cpf, '')) = normalizar_documento(v_cpf_referencia)
              AND normalizar_documento(v_cpf_referencia) != ''
              AND chave_endereco(p.cep, p.cidade, p.estado, COALESCE(p.logradouro, p.rua), p.numero) != v_chave_referencia
          )
      )
...
-- 2. Função para unificar pedidos
...
BEGIN
    ...
    -- Marcar pedido secundário como unificado
    UPDATE pedidos
    SET 
        status = 'Unificado',
    ...
    -- Atualizar pedidos_consolidados_v3
...
SELECT 'Funcionalidade de Unificação (Endereço ou CPF) criada com sucesso!' as status;
