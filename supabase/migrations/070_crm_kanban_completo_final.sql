-- ================================================================
-- MIGRATION 070: CRM Kanban Completo - FINAL
-- Sistema completo de gestão de leads com Kanban
-- Inspirado em Bitrix24 e Pipedrive
-- ================================================================

-- ================================================================
-- 1. TABELA DE PIPELINES (Funis/Kanbans)
-- ================================================================
...
-- ================================================================
-- 4. TABELA DE LEADS (Base completa)
-- ================================================================
...
-- ================================================================
-- 9. TABELA DE RELACIONAMENTO LEAD x PRODUTOS
-- ================================================================
...
-- Função para registrar mudança de etapa no histórico
...
BEGIN
    IF OLD.etapa_id IS DISTINCT FROM NEW.etapa_id THEN
        ...
        INSERT INTO crm_historico (
            ...
            pipeline_id, etapa_origem_id, etapa_destino_id, metadata
        ) VALUES (
            NEW.id, 'movimentacao', 'Mudança de Etapa',
...
BEGIN
    ...
    WHERE l.ativo = true AND e.sla_horas IS NOT NULL
      AND l.data_entrada_etapa < now() - (e.sla_horas || ' hours')::INTERVAL
      AND e.alerta_sla = true;
END;
$$ LANGUAGE plpgsql;

-- Função para atualizar histórico de compras
...
BEGIN
    ...
    UPDATE crm_leads SET historico_compras = v_historico WHERE id = p_lead_id;
END;
$$ LANGUAGE plpgsql;
...
('Recuperação', 'Recuperação de carrinhos abandonados', '#f59e0b', 3)
...
BEGIN
    ...
    IF v_pipeline_id IS NOT NULL THEN
        ...
        (v_pipeline_id, 'Proposta', 'Proposta enviada', '#f59e0b', 3, 'manual', 60, 72),
        (v_pipeline_id, 'Negociação', 'Em negociação', '#ec4899', 4, 'manual', 80, NULL),
        (v_pipeline_id, 'Fechado', 'Negócio ganho', '#10b981', 5, 'finalizacao', 100, NULL),
...
('Desejo Proibido - Kit 3un', 'Kit com 3 unidades', 297.00, 'DP'),
('Bela Forma - Completo', 'Kit completo BF', 197.00, 'BF'),
('Bela Lumi - 30 dias', 'Cápsulas 30 dias', 149.00, 'BL'),
('Combo DP+BF', 'Combo completo', 397.00, 'Combo')
ON CONFLICT DO NOTHING;
...
DROP POLICY IF EXISTS "crm_tags_all" ON crm_tags;
DROP POLICY IF EXISTS "crm_leads_all" ON crm_leads;
DROP POLICY IF EXISTS "crm_lead_tags_all" ON crm_lead_tags;
DROP POLICY IF EXISTS "crm_historico_all" ON crm_historico;
DROP POLICY IF EXISTS "crm_tarefas_all" ON crm_tarefas;
DROP POLICY IF EXISTS "crm_produtos_all" ON crm_produtos;
DROP POLICY IF EXISTS "crm_lead_produtos_all" ON crm_lead_produtos;

-- Create policies
CREATE POLICY "crm_pipelines_all" ON crm_pipelines FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "crm_etapas_all" ON crm_etapas FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "crm_tags_all" ON crm_tags FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "crm_leads_all" ON crm_leads FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "crm_lead_tags_all" ON crm_lead_tags FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "crm_historico_all" ON crm_historico FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "crm_tarefas_all" ON crm_tarefas FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "crm_produtos_all" ON crm_produtos FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "crm_lead_produtos_all" ON crm_lead_produtos FOR ALL USING (true) WITH CHECK (true);
...
-- Comentários
COMMENT ON TABLE crm_pipelines IS 'Pipelines/Kanbans personalizáveis';
COMMENT ON TABLE crm_etapas IS 'Colunas/Etapas de cada pipeline';
COMMENT ON TABLE crm_tags IS 'Tags para categorização de leads';
COMMENT ON TABLE crm_leads IS 'Base de leads com posicionamento Kanban';
COMMENT ON TABLE crm_historico IS 'Timeline de ações dos leads';
COMMENT ON TABLE crm_tarefas IS 'Tarefas e follow-ups';
COMMENT ON TABLE crm_produtos IS 'Catálogo de produtos/serviços';
