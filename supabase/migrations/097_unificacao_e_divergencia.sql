-- ================================================================
-- MIGRATION 097: NOVA LÓGICA DE UNIFICAÇÃO E DIVERGÊNCIAS
-- Data: 2026-03-19
-- 
-- Mudanças:
-- 1. Colunas tem_divergencia e itens_divergentes em pedidos_consolidados_v3
-- 2. Check de 4 pontos (Nome, Email, Telefone, CPF) na consolidar_pedidos_ticto()
-- 3. Cálculo de quantidade real (item_quantity)
-- 4. Novo padrão de nomenclatura de ofertas
-- ================================================================

-- Passo 1: Adicionar colunas necessárias
ALTER TABLE pedidos_consolidados_v3 
ADD COLUMN IF NOT EXISTS tem_divergencia BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS itens_divergentes JSONB DEFAULT '[]'::JSONB;

-- Passo 2: Atualizar a função de consolidação
...
BEGIN
    ...
    -- Proteção de itens bloqueados (etiquetados ou editados)
    ...
    FOR pedido_pai IN
        ...
        WHERE p.status IN ('authorized', 'approved', 'paid', 'completed', 'Aprovado', 'Autorizado', 'Pago')
          ...
          -- Filtro de pais (Regex expandida)
    ...
    LOOP
        ...
        -- Formatação Inicial Nome Oferta
        ...
        -- BUSCA DE FILHOS (Check 4 Pontos)
        FOR rec IN
            SELECT p.*,
                   REGEXP_REPLACE(p.customer_cpf, '[^0-9]', '', 'g') as doc_limpo,
            ...
            WHERE (
                ...
                OR (REGEXP_REPLACE(p.customer_cpf, '[^0-9]', '', 'g') = pedido_pai.doc_limpo)
            )
        ...
        LOOP
            -- Check de Divergência (Nome, Email, Telefone)
            IF (LOWER(TRIM(rec.customer_name)) != LOWER(TRIM(pedido_pai.customer_name)) OR
                ...
                v_lista_divergencias := v_lista_divergencias || jsonb_build_object(
                    ...
                    'nome', rec.customer_name,
                    'email', rec.customer_email,
                    'fone', rec.customer_phone,
                    'oferta', rec.offer_name
                );
                -- Não unifica automaticamente se divergir (pula este loop)
                CONTINUE;
            END IF;

            -- Se passou no check, unifica:
            ...
            ELSIF UPPER(rec.offer_name) ~ 'CC' THEN
                DECLARE 
                    v_sigla_pv TEXT := CASE 
                        ...
                        ELSE 'OUTRO' END;
                BEGIN
                    v_pos_vendas := array_append(v_pos_vendas, v_sigla_pv || ':' || rec.item_quantity || ':' || rec.offer_name);
                END;
            END IF;
        ...
        -- CONSTRUIR NOME DA OFERTA (Novo Padrão)
        -- Sigla - Nome Oferta
        v_descricao := pedido_pai.sigla || ' - ' || v_nome_oferta;

        IF array_length(v_order_bumps, 1) > 0 THEN
            v_descricao := v_descricao || ' + Order Bump';
        END IF;
        
        IF array_length(v_upsells, 1) > 0 THEN
            v_descricao := v_descricao || ' + UPSELL';
        END IF;

        -- Detalhamento Pós-Venda: + X SIGLA
        FOR rec IN SELECT unnest(v_pos_vendas) as item LOOP
            DECLARE 
                v_pv_sigla TEXT := SPLIT_PART(rec.item, ':', 1);
                v_pv_qtd TEXT := SPLIT_PART(rec.item, ':', 2);
            BEGIN
                IF v_pv_sigla != 'OUTRO' THEN
                    v_descricao := v_descricao || ' + ' || v_pv_qtd || ' ' || v_pv_sigla;
                END IF;
            END;
        END LOOP;
        ...
        INSERT INTO pedidos_consolidados_v3 (
            ...
            v_tem_divergencia, v_lista_divergencias
        )
        ON CONFLICT (codigo_transacao) DO UPDATE SET
            ...
            -- Preserva dia_despacho se já liberado
