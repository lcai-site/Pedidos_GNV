-- ================================================================
-- MIGRATION 126: FIX PV + COMBO DP
-- ================================================================
-- PROBLEMAS RESOLVIDOS:
--   1. Combos "Gota + Cápsula" (ex: "DP 1 Gota + 1 Capsula") eram
--      catalogados incorretamente como "DP - 1 UN CPS" porque a regex
--      não cobria a variante "Capsula" (sem acento/abreviação).
--      Fix: adicionado |CÁP a todas as verificações de Cápsula.
--
--   2. Pós-Vendas (filhos que não são OB nem Upsell) eram silenciosamente
--      ignorados porque a condição usava `ELSIF offer_name ~* '\mCC\M'`,
--      que nunca era verdade para a maioria dos pedidos.
--      Fix: substituído por ELSE — tudo que não for OB nem Upsell vira PV.
--
-- NOTA: Esta migration substitui o trecho equivalente da migration 107.
-- ================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- FIX 126: regex agora cobre CÁP além de CÁPSULA/CAPSULA
        -- ──────────────────────────────────────────────────────────
        DECLARE
        ...
        BEGIN
            IF pedido_pai.sigla = 'DP' THEN
                ...
                IF v_raw_offer IS NOT NULL THEN
                    ...
                    -- FIX 126: Adicionado |CÁP para cobrir "Capsula" e "Cáp" sem acento completo
        ...
        -- ──────────────────────────────────────────────────────────
        -- SUB-LOOP: Buscar filhos (Bumps, Upsells, PVs, 2 cartões)
        -- ──────────────────────────────────────────────────────────
        FOR rec IN
        ...
        LOOP
            ...
            -- FIX 126: ELSE captura todos os filhos restantes como PV
            --          (antes, precisava ter 'CC' no nome para entrar aqui)
            -- ──────────────────────────────────────────────────────────
            DECLARE
            ...
            BEGIN
                ...
                -- FIX 126: Adicionado |CÁP para cobrir "Capsula" sem acento completo
                ...
                ELSE
                    -- FIX 126: Tudo que não é OB nem Upsell é Pós-Venda
