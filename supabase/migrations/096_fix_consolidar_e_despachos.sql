-- ================================================================
-- MIGRATION 096: FIX CRÍTICO - Restaurar lógica correta de dia_despacho
-- Data: 2026-03-19
-- 
-- Problemas corrigidos:
-- 1. migration 20260319124500_fix_unificacao_v4.sql sobrescreveu a
--    consolidar_pedidos_ticto() com dia_despacho = CURRENT_DATE sempre.
--    Restauramos a lógica correta usando proximo_dia_util().
-- 2. ON CONFLICT sobrescrevia dia_despacho mesmo com pv_realizado=TRUE.
-- 3. Recalcula os dia_despacho incorretos já gravados hoje.
-- ================================================================

-- ============================================================
-- PASSO 1: RECALCULAR dia_despacho INCORRETOS JÁ NO BANCO
-- Pedidos consolidados hoje (19/03) com dia_despacho = hoje (19/03)
-- que NÃO foram liberados manualmente (pv_realizado IS NOT TRUE)
-- estão incorretos — devem ter dia_despacho = proximo_dia_util(proximo_dia_util(status_date))
-- ============================================================
...
-- ============================================================
-- PASSO 2: RESTAURAR consolidar_pedidos_ticto() CORRETA
-- Baseada na migration 095 (multi-plataforma), com as proteções:
-- - ON CONFLICT não sobrescreve dia_despacho de pv_realizado=TRUE
-- - Usa proximo_dia_util() para calcular datas corretamente
-- ============================================================
...
BEGIN
    ...
    LOOP
        ...
        -- *** CÁLCULO CORRETO DE DATAS usando proximo_dia_util() ***
        -- dia_pv  = próximo dia útil após a confirmação do pagamento
        -- dia_despacho = próximo dia útil após o dia_pv
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Fallback simples se proximo_dia_util não existir por algum motivo
        ...
        -- ON CONFLICT: NUNCA sobrescreve dia_despacho de pedidos com
        -- pv_realizado=TRUE (PV já feito) — preserva liberação manual
        -- ============================================================
        ...
        DO UPDATE SET
            ...
            -- *** PROTEÇÃO CRÍTICA: não sobrescreve dia_despacho se pv_realizado=TRUE ***
            dia_despacho = CASE 
                WHEN pedidos_consolidados_v3.pv_realizado = TRUE THEN pedidos_consolidados_v3.dia_despacho
                ELSE EXCLUDED.dia_despacho
            END,
            updated_at = NOW()
...
SELECT 'Migration 096: Lógica de dia_despacho restaurada e dados incorretos recalculados!' as status;
