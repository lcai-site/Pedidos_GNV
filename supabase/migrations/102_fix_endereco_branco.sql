-- ================================================================
-- MIGRATION 102: RESTAURANDO MAPA DE ENDEREÇOS NO CONSOLIDADOR
-- ================================================================
...
BEGIN
    ...
    JOIN ticto_pedidos p ON p.transaction_hash = c.codigo_transacao OR p.codigo_unico = ANY(codigos_agrupados)
    ...
    INSERT INTO _processed (id) SELECT id FROM _locked_codes;
    ...
    LOOP
        ...
        v_nome_oferta := REGEXP_REPLACE(v_nome_oferta, '^(DP|BF|BL)\s*-\s*', '', 'i');
        v_nome_oferta := REGEXP_REPLACE(v_nome_oferta, '\s*-\s+\w+\s*$', '', 'i');
        v_nome_oferta := REGEXP_REPLACE(v_nome_oferta, '(?i)\s*frascos\b', '', 'i');
        ...
        FOR rec IN
            ...
            WHERE (
                    ...
                    OR (pedido_pai.doc_limpo IS NOT NULL AND REGEXP_REPLACE(p.customer_cpf, '[^0-9]', '', 'g') = pedido_pai.doc_limpo)
                    OR (pedido_pai.transaction_hash IS NOT NULL AND p.transaction_hash = pedido_pai.transaction_hash)
                  )
        ...
        LOOP
            ...
            IF v_decisao_existente IS NULL AND rec.transaction_hash != pedido_pai.transaction_hash THEN
                IF LOWER(TRIM(COALESCE(rec.customer_name,''))) != LOWER(TRIM(COALESCE(pedido_pai.customer_name,''))) OR
                   LOWER(TRIM(COALESCE(rec.customer_email,''))) != LOWER(TRIM(COALESCE(pedido_pai.customer_email,''))) OR
                   REGEXP_REPLACE(COALESCE(rec.customer_phone,''), '[^0-9]', '', 'g') != REGEXP_REPLACE(COALESCE(pedido_pai.customer_phone,''), '[^0-9]', '', 'g') THEN
                    
                    v_tem_divergencia := TRUE;
                    v_lista_divergencias := v_lista_divergencias || jsonb_build_object('hash', rec.transaction_hash, 'codigo_unico', rec.codigo_unico, 'nome', rec.customer_name, 'email', rec.customer_email, 'fone', rec.customer_phone, 'oferta', rec.offer_name, 'produto', rec.product_name, 'quantidade', rec.item_quantity);
                    CONTINUE;
                END IF;
            END IF;
        ...
        v_endereco_completo := COALESCE(pedido_pai.address_street, '') || ', ' || COALESCE(pedido_pai.address_number, '') || 
                               CASE WHEN pedido_pai.address_complement IS NOT NULL AND pedido_pai.address_complement != '' THEN ' - ' || pedido_pai.address_complement ELSE '' END || 
                               ' - ' || COALESCE(pedido_pai.address_neighborhood, '') || ', ' || COALESCE(pedido_pai.address_city, '') || '/' || COALESCE(pedido_pai.address_state, '') || 
        ...
        INSERT INTO pedidos_consolidados_v3 (
            ...
            v_descricao, ARRAY[pedido_pai.codigo_unico] || v_codigos_filhos, v_quantidade, pedido_pai.sigla,
            ...
            pedido_pai.address_street, pedido_pai.address_number, pedido_pai.address_complement, pedido_pai.address_neighborhood, pedido_pai.address_city, pedido_pai.address_state, v_endereco_completo
            ...
            bairro = EXCLUDED.bairro, cidade = EXCLUDED.cidade, estado = EXCLUDED.estado, endereco_completo = EXCLUDED.endereco_completo,
            cep = EXCLUDED.cep, telefone = EXCLUDED.telefone, nome_cliente = EXCLUDED.nome_cliente, cpf = EXCLUDED.cpf,
